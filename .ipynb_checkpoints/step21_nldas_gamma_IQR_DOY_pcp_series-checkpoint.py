...
output_file1 = 'ppf25_pcp_series.txt'
output_file2 = 'ppf75_pcp_series.txt'
output_file3 = 'IQR_pcp_series.txt'
...
for i in range(2):
    ...
    for j in range(nx):
        if ~np.isnan(alpha[i,j]):
            q25[i,j] = gamma.ppf(0.25, alpha[i,j], loc=0, scale=beta[i,j])
            q75[i,j] = gamma.ppf(0.75, alpha[i,j], loc=0, scale=beta[i,j])
        
#         q25[i,j] = [pool.apply(ppf, args=(0.25, alpha[i,j], beta[i,j])) for j in range(nx)]
#         q75[i,j] = [pool.apply(ppf, args=(0.75, alpha[i,j], beta[i,j])) for j in range(nx)]
